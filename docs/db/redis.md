...
**周期删除**：顾名思义是通过一个定时任务，周期性的**抽样部分过期的 key**，然后执行删除。执行周期有两种：
...
4. 如果没有达到时间上限（25ms）并且过期 key 比例大于 10%，再进行一次抽样，否则结束
...
2. 执行清理耗时不超过 1ms
3. 逐个遍历 db，逐个遍历 db 中的 bucket，抽取 20 个 key 判断是否过期
4. 如果没有达到时间上限（1ms）并且过期 key 比例大于 10%，再进行一次抽样，否则结束
